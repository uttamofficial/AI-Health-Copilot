  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2] !== undefined)
      ...
      parts.push(
        <code
          key={match.index}
          className="bg-blue-50 text-blue-700 px-1 rounded text-xs font-mono"
        >
          {match[4]}
        </code>,
      );
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith("⚠️")) {
      blocks.push(
        <div
          key={i}
          className="my-2 p-2 bg-amber-50 border-l-4 border-amber-400 rounded-r-lg text-xs text-amber-800"
        >
    ...
    if (line.trimStart().startsWith("> ")) {
      blocks.push(
        <div
          key={i}
          className="my-1 pl-3 border-l-4 border-blue-300 text-gray-600 italic text-xs"
        >
    ...
    if (/^[\s]*[-*]\s/.test(line)) {
      ...
      blocks.push(
        <ul key={`ul-${i}`} className="my-1 space-y-1 text-xs">
          {items}
        </ul>,
      );
      continue;
    ...
    if (/^[\s]*\d+\.\s/.test(line)) {
      ...
      while (i < lines.length && /^[\s]*\d+\.\s/.test(lines[i])) {
        items.push(
          <li key={i} className="flex gap-1.5 items-start">
            <span className="flex-shrink-0 w-4 h-4 rounded-full bg-blue-100 text-blue-700 text-[10px] font-bold flex items-center justify-center">
              {num}
            </span>
      ...
      blocks.push(
        <ol key={`ol-${i}`} className="my-1 space-y-1 text-xs">
          {items}
        </ol>,
      );
      continue;
      ...
      {isOpen && (
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, #0040C1 0%, #3b5fe2 50%, #6366f1 100%)",
            }}
          >
            {/* Decorative circles */}
              ...
              >
                ...
                {message.sender === "bot" && (
                  <div
                    ...
                    style={{
                      background: "linear-gradient(135deg, #0040C1, #6366f1)",
                    }}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                <div
                  className={`max-w-[78%] ${message.sender === "user" ? "items-end" : "items-start"} flex flex-col`}
                >
                  <div
                    ...
                    style={
                      message.sender === "user"
                        ? {
                            background:
                              "linear-gradient(135deg, #0040C1, #3b5fe2)",
                          }
                        : {}
                    }
                  >
          ...
          <div className="px-4 pb-4 pt-3 bg-white/80 backdrop-blur-sm border-t border-blue-50 flex-shrink-0">
                ...
                style={{
                  background:
                    inputValue.trim() && !isLoading
                      ? "linear-gradient(135deg, #0040C1, #6366f1)"
                      : "#e5e7eb",
                }}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`h-4 w-4 ${inputValue.trim() && !isLoading ? "text-white" : "text-gray-400"}`}
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
            ...
            <p className="text-[10px] text-gray-400 text-center mt-2">
              🔒 AI Health Copilot · Not a substitute for professional medical
              advice
            </p>
          </div>
        </div>
